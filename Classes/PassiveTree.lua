...
local PassiveTreeClass = newClass("PassiveTree", function(self, treeVersion)
	...
	for _, node in pairs(self.nodes) do
		...
		if group then
			...
			if node.o ~= 4 then
				node.angle = node.oidx * self:getOrbitMult()[node.o]
			else
				node.angle = self:getOrbitMultFull()[node.oidx]
			end
			local dist = self:getOrbitDist()[node.o]
...
function PassiveTreeClass:getOrbitMult()
	return { [0] = 0, m_pi / 3, m_pi / 6, m_pi / 6 }
end

function PassiveTreeClass:getOrbitMultFull()
	return {
		[0] = 0,
		[1] = 10 * m_pi / 180,
		[2] = 20 * m_pi / 180,
...
function PassiveTreeClass:getOrbitDist()
	return { [0] = 0, 82, 162, 335, 493 }
end
