describe("TradeHelpers trade hash matching", function()
	...
	describe("newPlainNumericEdit", function()
		it("sets correct filter pattern for integers", function()
			local ctrl = tradeHelpers.newPlainNumericEdit(nil, { 0, 0, 100, 20 }, "", "Min", 6, true)
			assert.equal("^%-%d", ctrl.filter)
			assert.equal("[^%-%d]", ctrl.filterPattern)
		end)

		it("sets correct filter pattern for floats", function()
			local ctrl = tradeHelpers.newPlainNumericEdit(nil, { 0, 0, 100, 20 }, "", "Min", 6, false)
			assert.equal("^%-%d.", ctrl.filter)
			assert.equal("[^%-%d.]", ctrl.filterPattern)
		end)
	end)
