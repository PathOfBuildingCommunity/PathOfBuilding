...
describe("RadiusJewelFinder actions #radius-jewel", function()
	...
	it("offers both actions and explains that an unallocated socket is hidden from Items", function()
		...
		assert.is_true(equipTooltip:find("Current location:", 1, true) ~= nil)
		assert.is_true(equipTooltip:find("Not in build", 1, true) ~= nil)
		...
		assert.is_true(details:find("Current location:", 1, true) ~= nil)
		assert.is_true(details:find("Not in build", 1, true) ~= nil)
		assert.is_nil(details:find("Source:", 1, true))
		assert.is_nil(details:find("This socket is unallocated", 1, true))
		assert.is_nil(details:find("hidden from the Items panel", 1, true))
		assert.is_nil(details:find("not applied automatically", 1, true))
	end)
	...
	it("shows Equipped and disables the action for an exact Thread ring", function()
		...
		local details = listText(popup.controls.resultDetailList)
		assert.is_true(details:find("Current location:", 1, true) ~= nil)
		assert.is_true(details:find("This socket", 1, true) ~= nil)
	end)
	...
	it("moves a limited jewel while preserving an occupied target for Undo", function()
		...
		assert.is_true(actionTooltip:find("Current location:", 1, true) ~= nil)
		...
		assert.is_nil(details:find("Move equipped jewel", 1, true))
		assert.is_nil(details:find("not applied automatically", 1, true))
	...
	it("invalidates an Items source that is socketed after planning", function()
		...
		local details = listText(popup.controls.resultDetailList)
		assert.is_true(details:find("Current location:", 1, true) ~= nil)
		assert.is_true(details:find("Items", 1, true) ~= nil)
		assert.are.equal("In build", popup.controls.addToBuildButton:GetProperty("label"))
		assert.is_false(popup.controls.addToBuildButton.enabled())
		assert.is_true(popup.controls.applyButton.enabled())
