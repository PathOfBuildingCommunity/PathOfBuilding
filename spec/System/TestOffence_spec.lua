describe("TestOffence", function()
	...
	it("applies Fist of War to player Ground Slam", function()
		build.itemsTab:CreateDisplayItemFromRaw("New Item\nDriftwood Maul")
		build.itemsTab:AddDisplayItem()
		build.skillsTab:PasteSocketGroup("Ground Slam 20/0  1\nFist of War 20/0  1\n")
		runCallback("OnFrame")
		assert.is_true(build.calcsTab.mainEnv.player.mainSkill.skillTypes[SkillType.Slam])
		assert.is_true(build.calcsTab.mainOutput.FistOfWarUptimeRatio > 0)
	end)

	it("does not apply Fist of War to player Cleave", function()
		build.itemsTab:CreateDisplayItemFromRaw("New Item\nRusted Hatchet")
		build.itemsTab:AddDisplayItem()
		build.skillsTab:PasteSocketGroup("Cleave 20/0  1\nFist of War 20/0  1\n")
		runCallback("OnFrame")
		assert.is_true(not build.calcsTab.mainEnv.player.mainSkill.skillTypes[SkillType.Slam])
		assert.is_true(not build.calcsTab.mainOutput.FistOfWarUptimeRatio or build.calcsTab.mainOutput.FistOfWarUptimeRatio == 0)
	end)

	it("does not apply injected Fist of War mods to player Cleave", function()
		build.itemsTab:CreateDisplayItemFromRaw("New Item\nRusted Hatchet")
		build.itemsTab:AddDisplayItem()
		build.skillsTab:PasteSocketGroup("Cleave 20/0  1\n")
		build.configTab:BuildModList()
		build.configTab.modList:NewMod("FistOfWarCooldown", "BASE", 1, "Test")
		build.configTab.modList:NewMod("FistOfWarDamageMultiplier", "BASE", 100, "Test")
		build.calcsTab:BuildOutput()
		local skill = build.calcsTab.mainEnv.player.mainSkill
		assert.is_true(not skill.skillTypes[SkillType.Slam])
		assert.is_true(skill.skillModList:Sum("BASE", skill.skillCfg, "FistOfWarCooldown") > 0)
		assert.is_true(not build.calcsTab.mainOutput.FistOfWarUptimeRatio or build.calcsTab.mainOutput.FistOfWarUptimeRatio == 0)
	end)
end)
