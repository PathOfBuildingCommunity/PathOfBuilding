...
local result = gimpBatch.combine_images_to_sprite(
	"legion-active", { filename = "legion-active.png", w = iconWidth, h = iconHeight, coords = iconEntries },
	outputPath, temporaryPath, combineImagesScript, 100
)
assert(result == 0 or result == true, "GIMP failed to create legion-active.png")
result = gimpBatch.combine_images_to_sprite(
	"legion-inactive", { filename = "legion-inactive.png", w = iconWidth, h = iconHeight, coords = iconEntries },
	outputPath, temporaryPath, combineImagesScript, 40
)
assert(result == 0 or result == true, "GIMP failed to create legion-inactive.png")

result = gimpBatch.extract_abyss_lines(
	connectionSources, temporaryPath, outputPath, temporaryPath .. "legion-lines.scm"
)
assert(result == 0 or result == true, "GIMP failed to extract the Abyss connection art")
...
result = gimpBatch.combine_images_to_sprite(
	"legion-art", { filename = "legion-art.png", w = artWidth, h = artHeight, coords = frameEntries },
	outputPath, temporaryPath, combineImagesScript, 100
)
assert(result == 0 or result == true, "GIMP failed to create legion-art.png")
