	...
	if (base + extraBase) ~= 0 then
		...
		if inc ~= 0 or more ~= 1 or (base ~= 0 and extraBase ~= 0) then
			...
			if base ~= 0 and extraBase ~= 0 then
				out[1] = c_format("(%g + %g) {TEXT_SECONDARY}(base)", extraBase, base)
			else
				out[1] = c_format("%g {TEXT_SECONDARY}(base)", base + extraBase)
			end
			if inc ~= 0 then
				t_insert(out, c_format("x %.2f {TEXT_SECONDARY}(increased/reduced)", 1 + inc/100))
			end
			if more ~= 1 then
				t_insert(out, c_format("x %.2f {TEXT_SECONDARY}(more/less)", more))
			end
...
function breakdown.area(base, areaMod, total, incBreakpoint, moreBreakpoint, redBreakpoint, lessBreakpoint, label)
	...
	t_insert(out, label)
	if base ~= total then
		t_insert(out, c_format("%.1fm {TEXT_SECONDARY}(base radius)", base / 10))
		t_insert(out, c_format("x %.2f {TEXT_SECONDARY}(square root of area of effect modifier)", m_floor(100 * m_sqrt(areaMod)) / 100))
		t_insert(out, s_format("= %.1fm", total / 10))
	end
	if incBreakpoint and moreBreakpoint and redBreakpoint and lessBreakpoint then
		t_insert(out, c_format("{TEXT_SECONDARY}Next 0.1m breakpoint: %d%% increased AoE / a %d%% more AoE multiplier", incBreakpoint, moreBreakpoint))
		t_insert(out, c_format("{TEXT_SECONDARY}Previous 0.1m breakpoint: %d%% reduced AoE / a %d%% less AoE multiplier", redBreakpoint, lessBreakpoint))
	end
	out.radius = total
	return out
end
...
function breakdown.effMult(damageType, resist, pen, taken, mult, takenMore, sourceRes, useRes, invertChance)
	...
	if sourceRes and sourceRes ~= damageType then
		t_insert(out, c_format("Enemy %s: %d%% {TEXT_SECONDARY}(%s)", resistLabel, resist, sourceRes))
	...
	if pen ~= 0 or not useRes then
		...
		t_insert(out, c_format("%d%% {TEXT_SECONDARY}(resistance)", resist))
		if pen < 0 then
			t_insert(out, c_format("+ %d%% {TEXT_SECONDARY}(penetration)", -pen))
		elseif pen > 0 then
			t_insert(out, c_format("- %d%% {TEXT_SECONDARY}(penetration)", pen))
		end
		if not useRes then
			t_insert(out, c_format("x %d%% {TEXT_SECONDARY}(resistance ignored)", 0))
	...
	if useRes then
		breakdown.multiChain(out, {
			...
			{ "%.2f {TEXT_SECONDARY}(%s)", 1 - (resist - pen) / 100, resistForm },
			{ "%.2f {TEXT_SECONDARY}(increased/reduced damage taken)", 1 + taken / 100 },
			{ "%.2f {TEXT_SECONDARY}(more/less damage taken)", takenMore },
			total = s_format("= %.3f", mult),
		})
	else
		t_insert(out, "Effective DPS modifier:")
		t_insert(out, c_format("= %.3f {TEXT_SECONDARY}(increased/reduced damage taken)", mult))
	end
	return out
end
...
function breakdown.dot(out, baseVal, inc, more, mult, rate, aura, effMult, total)
	breakdown.multiChain(out, {
		base = { "%.1f {TEXT_SECONDARY}(base damage per second)", baseVal }, 
		{ "%.2f {TEXT_SECONDARY}(increased/reduced)", 1 + inc/100 },
		{ "%.2f {TEXT_SECONDARY}(more/less)", more },
		{ "%.2f {TEXT_SECONDARY}(multiplier)", 1 + (mult or 0)/100 },
...
function breakdown.critDot(dotMulti, critMulti, dotChance, critChance)
	...
	if dotChance > 0 then
		...
		t_insert(out, c_format("%.2f {TEXT_SECONDARY}(dot multiplier for non-crits)", dotMulti))
		t_insert(out, c_format("x %.4f {TEXT_SECONDARY}(portion of instances created by non-crits)", dotChance))
		t_insert(out, s_format("= %.2f", dotMulti * dotChance))
	end
	if critChance > 0 then
		t_insert(out, s_format("Contribution from Crits:"))
		t_insert(out, c_format("%.2f {TEXT_SECONDARY}(dot multiplier for crits)", critMulti))
		t_insert(out, c_format("x %.4f {TEXT_SECONDARY}(portion of instances created by crits)", critChance))
	...
	if actor.mainSkill.skillData.showAverage then
		if instant > 0 then
			if instantLeechProportion ~= 1 then 
				t_insert(out, c_format("Instant Leech: %.1f {TEXT_SECONDARY}(%d%% x %.1f)", instant, instantLeechProportion * 100, dur * pool * data.misc.LeechRateBase / (1-instantLeechProportion)))
			else
		...
		if instances > 0 then
			...
			t_insert(out, c_format("%d {TEXT_SECONDARY}(size of leech destination pool)", pool))
			t_insert(out, c_format("x %.2f {TEXT_SECONDARY}(base leech rate is %d%% per second)", data.misc.LeechRateBase, 100 * data.misc.LeechRateBase))
			local rateMod = calcLib.mod(modDB, skillCfg, rate)
			if rateMod ~= 1 then
				t_insert(out, c_format("x %.2f {TEXT_SECONDARY}(leech rate modifier)", rateMod))
			end
			t_insert(out, c_format("x %.2fs {TEXT_SECONDARY}(instance duration)", dur))
		...
		if instantRate > 0 then
			...
			t_insert(out, c_format("Instant Leech per second: %.1f {TEXT_SECONDARY}(%.1f x %.2f)", instantRate, instant, hitRate))
		end
		if instances > 0 then
			...
			t_insert(out, c_format("= %.1f {TEXT_SECONDARY}per second", pool * data.misc.LeechRateBase * rateMod))
			t_insert(out, "Maximum leech rate against one target:")
			t_insert(out, s_format("%.1f", pool * data.misc.LeechRateBase * rateMod))
			t_insert(out, c_format("x %.2f {TEXT_SECONDARY}(average instances)", instances))
			local total = pool * data.misc.LeechRateBase * rateMod * instances
			t_insert(out, c_format("= %.1f {TEXT_SECONDARY}per second", total))
			if total <= max then
