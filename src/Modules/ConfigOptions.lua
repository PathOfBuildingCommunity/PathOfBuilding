...
return {
	...
	{ var = "embraceMadnessActive", type = "check", label = "Is Embrace Madness active?", ifSkill = "Embrace Madness", defaultState = true, apply = function(val, modList, enemyModList, build)
		affectedByGloriousMadness = false
		-- is Embrace Madness skill used
		if build.calcsTab.mainEnv and build.calcsTab.mainEnv.skillsUsed then
			for skill,_ in pairs(build.calcsTab.mainEnv.skillsUsed) do
				if calcLib.isGemIdSame(skill, "Embrace Madness", true) then
					affectedByGloriousMadness = true
				end
			end
		end
		modList:NewMod("Condition:AffectedByGloriousMadness", "FLAG", affectedByGloriousMadness, "Config")
	end },
