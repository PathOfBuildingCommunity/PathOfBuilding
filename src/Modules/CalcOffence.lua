...
function calcs.offence(env, actor, activeSkill)
	...
	for _, pass in ipairs(passList) do
		...
		if canDeal.Physical and (output.BleedChanceOnHit + output.BleedChanceOnCrit) > 0 then
			...
			local sourceHitDmg, sourceCritDmg, sourceMinHitDmg, sourceMaxHitDmg
			if breakdown then
			...
			if globalBreakdown then
				globalBreakdown.BleedStackPotential = {
					...
					s_format("* (%.2f / %.2f) ^8(Bleed duration / Attack Time)", globalOutput.BleedDuration, (output.HitTime or output.Time)),
				}
				if skillData.dpsMultiplier ~= 1 then
		...
		if canDeal.Fire and (output.IgniteChanceOnHit + output.IgniteChanceOnCrit) > 0 then
			...
			globalOutput.IgniteStackPotential = igniteStacks or 1
			if globalBreakdown then
				...
				elseif output.Cooldown then
					t_insert(globalBreakdown.IgniteStackPotential, s_format("(%.2f / max(%.2f, %.2f) ^8(IgniteDuration / max(Cooldown, Cast Time))", globalOutput.IgniteDuration, output.Cooldown, (output.HitTime or output.Time)))
				else
					t_insert(globalBreakdown.IgniteStackPotential, s_format("(%.2f / %.2f) ^8(IgniteDuration / Cast Time)", globalOutput.IgniteDuration, (output.HitTime or output.Time)))
				end
				if skillData.dpsMultiplier ~= 1 then
