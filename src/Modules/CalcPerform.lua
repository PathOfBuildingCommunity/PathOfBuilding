...
local function doActorMisc(env, actor)
	...
	if env.mode_combat then
		...
		if modDB:Flag(nil, "Chill") then
			local effect = m_min(m_max(m_floor((modDB:Override(nil, "ChillVal") or calcLib.val(modDB, "ChillVal")) *  (calcLib.mod(modDB, nil, "SelfChillEffect") + ((modDB:Flag(nil, "SelfChillEffectIsReversed") and calcLib.mod(modDB, nil, "EnemyChillEffect") - 1) or 0))), 0), 30)
