...
local AtlasSpecClass = newClass("AtlasSpec", "UndoHandler", function(self, build, treeVersion)
	self.UndoHandler()
	...
	-- Have these to reduce errors in SpecListControl
...
function AtlasSpecClass:Save(xml)
...
function AtlasSpecClass:AllocateMasteryEffects(masteryEffects)
...
function AtlasSpecClass:ResetNodes()
	for id, node in pairs(self.nodes) do
...
function AtlasSpecClass:AllocNode(node, altPath)
...
function AtlasSpecClass:DeallocNode(node)
...
function AtlasSpecClass:AddMasteryEffectOptionsToNode(node)
	node.sd = {}
...
function AtlasSpecClass:BuildAllDependsAndPaths()
...
function AtlasSpecClass:ReplaceNode(old, newNode)
...
function AtlasSpecClass:ReconnectNodeToClassStart(node)
...
function AtlasSpecClass:BuildClusterJewelGraphs()
	...
	self.build.atlasTab.viewer.searchStrCached = ""
end

function AtlasSpecClass:BuildSubgraph(jewel, parentSocket, id, upSize, importedNodes, importedGroups)
...
function AtlasSpecClass:CreateUndoState()
	local allocNodeIdList = { }
	for nodeId in pairs(self.allocNodes) do
		t_insert(allocNodeIdList, nodeId)
	end
...
function AtlasSpecClass:RestoreUndoState(state)
...
function AtlasSpecClass:SetWindowTitleWithBuildClass()
...
function AtlasSpecClass:NodeAdditionOrReplacementFromString(node,sd,replacement)
	local addition = {}
	addition.sd = {sd}
	addition.mods = { }
