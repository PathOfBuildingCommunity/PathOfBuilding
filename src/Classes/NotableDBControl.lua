...
function NotableDBClass:ListBuilder()
	...
	if self.sortDetail and self.sortDetail.stat then -- stat-based
		...
		local calcBase = calcFunc(self.itemsTab:ItemCalculationOverride(itemType, self.itemsTab:anointItem(nil)))
		self.sortMaxPower = 0
		for nodeIndex, node in ipairs(list) do
			node.measuredPower = 0
			if node.modKey ~= "" then
				local output = calcFunc(self.itemsTab:ItemCalculationOverride(itemType, self.itemsTab:anointItem(node)))
