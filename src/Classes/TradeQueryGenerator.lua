...
-- pseudoMemberLookup maps tradeModId -> pseudoId.
...
local function normTradeText(text)
	...
	text = text:gsub("[+#%%]", "")
	text = text:gsub("%d+%.?%d*", "")
	text = text:gsub("%s*%(local%)%s*", " ")
	text = text:gsub("%s*%(global%)%s*", " ")
	text = text:gsub("%f[%a]total%f[%A]", "")
...
function TradeQueryGeneratorClass:InitMods()
	...
	if file then
		...
		-- Load cached pseudo stat mapping if available; otherwise build it from a fresh API fetch
		local psFile = io.open("Data/PseudoStats.lua","r")
		if psFile then
			psFile:close()
...
-- Builds pseudoMemberLookup from the PoE trade API stats data and saves it to Data/PseudoStats.lua.
-- Called once by InitMods() when the cache file doesn't exist yet.
buildAndCachePseudoMapping = function(tradeQueryStatsParsed)
	-- Build normalized text -> list of tradeModIds for all explicit/implicit entries
	local normTextToModIds = {}
	for _, modTypeEntry in ipairs(tradeQueryStatsParsed.result) do
		local label = modTypeEntry.label
		if label == "Explicit" or label == "Implicit" then
			for _, entry in ipairs(modTypeEntry.entries) do
				local norm = normTradeText(entry.text)
				if not normTextToModIds[norm] then
					normTextToModIds[norm] = {}
				end
				t_insert(normTextToModIds[norm], entry.id)
			end
		end
	end
	...
	for _, modTypeEntry in ipairs(tradeQueryStatsParsed.result) do
		if modTypeEntry.label == "Pseudo" then
			for _, pseudoEntry in ipairs(modTypeEntry.entries) do
				local pseudoId = pseudoEntry.id
				local norm = normTradeText(pseudoEntry.text)
				local memberIds = pseudoNormFallbacks[norm] or normTextToModIds[norm]
				if memberIds then
	...
	-- Save to cache
	local pseudoStatsFile = io.open("Data/PseudoStats.lua", 'w')
	if pseudoStatsFile then
		pseudoStatsFile:write("-- This file is automatically generated, do not edit!\n-- Stat data (c) Grinding Gear Games\n\n")
		pseudoStatsFile:write("return " .. stringify(pseudoMapping))
...
-- Consolidates modWeights entries that share a pseudo stat equivalent into a single entry.
-- weight  = average of members (1 unit of the pseudo stat is worth the same regardless of source)
-- meanStatDiff = sum of members (captures combined build impact for sort priority)
local function consolidateToPseudo(modWeights)
	local pseudoGroupData = {}
	local nonPseudo = {}

	for _, entry in ipairs(modWeights) do
		local pseudoId = pseudoMemberLookup[entry.tradeModId]
		if pseudoId then
			local g = pseudoGroupData[pseudoId]
			if not g then
				g = { totalWeight = 0, totalMeanStatDiff = 0, count = 0 }
				pseudoGroupData[pseudoId] = g
			end
			g.totalWeight = g.totalWeight + entry.weight
			g.totalMeanStatDiff = g.totalMeanStatDiff + entry.meanStatDiff
			g.count = g.count + 1
		else
			t_insert(nonPseudo, entry)
		end
	end

	local result = {}
	for pseudoId, g in pairs(pseudoGroupData) do
		t_insert(result, {
			tradeModId = pseudoId,
			weight = g.totalWeight / g.count,
			meanStatDiff = g.totalMeanStatDiff,
			invert = false,
		})
	end
	for _, e in ipairs(nonPseudo) do
		t_insert(result, e)
	end
...
function TradeQueryGeneratorClass:FinishQuery()
	...
	if requireCurrentMods and originalItem then
		...
		local catalystTagGroups = {
			{ "attack" }, { "speed" }, { "life", "mana", "resource" }, { "caster" },
			{ "jewellery_attribute", "attribute" }, { "physical_damage", "chaos_damage" },
			{ "jewellery_resistance", "resistance" }, { "jewellery_defense", "defences" },
			{ "jewellery_elemental", "elemental_damage" }, { "critical" },
		}
		local tradeModIdToTags = {}
		if originalItem.affixes then
			for _, mod in pairs(originalItem.affixes) do
				if mod.modTags and #mod.modTags > 0 and mod.statOrder and mod.group then
					for _, statOrder in ipairs(mod.statOrder) do
						local key = tostring(statOrder) .. "_" .. mod.group
						for _, modType in ipairs({ "Explicit", "Implicit" }) do
							local entry = self.modData[modType] and self.modData[modType][key]
							if entry then
								tradeModIdToTags[entry.tradeMod.id] = mod.modTags
							end
						end
					end
				end
		...
		-- For implicit mod lines, fall back to the explicit lookup — some mods that
		-- appear as implicits on items (e.g. Abyssal Socket) are only registered as
		-- explicit stats in modData. In that case, rewrite the "explicit." prefix to
		-- "implicit." so the trade filter targets the correct stat category.
			...
			for _, modLine in ipairs(modLines) do
				if not modLine.crafted then
					...
					if not tradeModId and fallbackLookup then
						tradeModId = fallbackLookup[matchStr]
						usedFallback = tradeModId ~= nil
					end
					if tradeModId then
						if usedFallback then
							tradeModId = tradeModId:gsub("^explicit%.", "implicit.")
						end
						local value = tonumber(modLine.line:match("(%d+%.?%d*)"))
						-- Deaugment catalyst-boosted values using tradeModId -> modTags lookup.
						-- This works regardless of prefix metadata or stored line text.
						if value and originalItem.catalyst and originalItem.catalystQuality then
							local modTags = tradeModIdToTags[tradeModId]
							if modTags then
								local affectedTags = catalystTagGroups[originalItem.catalyst]
								if affectedTags then
									local deaugmented = false
									for _, modTag in ipairs(modTags) do
										if deaugmented then break end
										for _, catTag in ipairs(affectedTags) do
											if modTag == catTag then
												value = math.floor(value / ((100 + originalItem.catalystQuality) / 100) + 0.5)
												deaugmented = true
												break
											end
										end
						...
						t_insert(requiredModFilters, { id = tradeModId, value = value and { min = value } or nil })
					end
				end
			end
		end
	...
	for _, reqFilter in ipairs(requiredModFilters) do
		t_insert(andFilters.filters, reqFilter)
		filters = filters + 1
	end
...
function TradeQueryGeneratorClass:RequestQuery(slot, context, statWeights, callback)
	...
	-- Mode dropdown: always visible, but with dynamic list based on item type
	-- Without item: Standard only
	-- With non-unique item: Standard/Upgrade/Exact/Implicit Upgrade
	-- With unique item: Standard/Implicit Upgrade (Upgrade and Exact not applicable to unique items)
	local modeListNoItem = {"Standard"}
	local modeListWithNonUnique = {"Standard", "Upgrade", "Exact", "Implicit Upgrade"}
	local modeListWithUnique = {"Standard", "Implicit Upgrade"}

	-- Check if the equipped item is actually a unique (check rarity, not slot type)
	local isUniqueItem = existingItemForSlot and existingItemForSlot.rarity == "UNIQUE"
	local modeList
	if not existingItemForSlot then
		modeList = modeListNoItem
	elseif isUniqueItem then
		modeList = modeListWithUnique
	else
		modeList = modeListWithNonUnique
	end

	local modeTooltip = "Determines search query groups:\n" ..
		"Standard: Search for BIS item - weighted group only\n" ..
		"Upgrade: Search for better item - exact mods + weighted group\n" ..
		"Exact: Search for similar item - exact mods only\n" ..
		"Implicit Upgrade: Search for better implicits - implicit only weighted group\n" ..
		"Modes Upgrade/Exact/Implicit Upgrade require item in slot"

	controls.tradeMode = new("DropDownControl", {"TOPLEFT",lastItemAnchor,"BOTTOMLEFT"}, {0, 5, 140, 18}, modeList, function(index, value) end)
	controls.tradeMode.tooltipText = modeTooltip
	-- Map the saved selection to the new list indices
	local savedMode = self.lastTradeMode or 1
	local selectedIndex = 1  -- default to Standard
	if isUniqueItem then
		-- For unique items: map modes 2,3,4 to index 2 (Implicit Upgrade)
		selectedIndex = (savedMode == 4) and 2 or 1
	elseif existingItemForSlot then
		-- For non-unique items with item: use saved mode as-is (1-4 maps to 1-4)
		selectedIndex = savedMode
	else
		-- Without item: always Standard (1)
		selectedIndex = 1
	end
	controls.tradeMode.selIndex = selectedIndex
	controls.tradeModeLabel = new("LabelControl", {"RIGHT",controls.tradeMode,"LEFT"}, {-5, 0, 0, 16}, "Mode:")
	updateLastAnchor(controls.tradeMode)

	-- Pseudo Mods checkbox: always visible
	...
	-- Implicit Mods dropdown: always visible with 4 options (None / Without Eldritch / Eldritch limited / All)
	...
	controls.includeImplicits:SetSel(self.lastIncludeImplicits or 4)  -- default: All
	updateLastAnchor(controls.includeImplicits)

	-- Copy Current Implicits checkbox: only visible for Eldritch-capable slots when item is equipped
	if existingItemForSlot and isEldritchModSlot then
		...
		controls.copyEldritch.tooltipText =
			...
			"Set Implicit Mods to None when using this — the Eldritch pool will be\n" ..
			"applied from your item rather than searched."
...
Remove: %s will be removed from the search results.]], term, term, term)
	...
	controls.generateQuery = new("ButtonControl", { "BOTTOM", nil, "BOTTOM" }, {-45, -10, 80, 20}, "Execute", function()
		...
		if controls.copyEldritch then
			...
			options.copyEldritch = controls.copyEldritch.state
		end
