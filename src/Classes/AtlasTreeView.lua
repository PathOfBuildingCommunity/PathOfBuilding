...
local AtlasTreeViewClass = newClass("AtlasTreeView", function(self)
...
function AtlasTreeViewClass:Draw(build, viewPort, inputEvents)
		...
		if hoverNode then
			...
			elseif hoverNode.path then
				...
				-- if hoverNode.type == "Mastery" and hoverNode.masteryEffects then
					-- build.atlasTab:OpenMasteryPopup(hoverNode, viewPort)
				-- else
	...
	-- elseif treeClick == "RIGHT" then
		...
		-- elseif hoverNode and hoverNode.conqueredBy and hoverNode.type ~= "Keystone" and
			...
			-- build.atlasTab:ModifyNodePopup(hoverNode, viewPort)
			-- build.buildFlag = true
	...
	local bg = tree.assets.AtlasPassiveBackground or tree.assets.Background1
	if bg.width == 0 then
	...
	for nodeId, node in pairs(spec.nodes) do
		...
		if node.startNode then
			overlay = "AtlasPassiveSkillScreenStart"
		else
			local state
			if isAlloc or node == hoverNode or (self.traceMode and node == self.tracePath[#self.tracePath])then
...
function AtlasTreeViewClass:DrawAsset(data, x, y, scale, isHalf)
	if not data then
		return
	end
...
function AtlasTreeViewClass:Zoom(level, viewPort)
...
function AtlasTreeViewClass:Focus(x, y, viewPort, build)
	self.zoomLevel = 12
	self.zoom = 1.2 ^ self.zoomLevel

	local tree = build.atlasSpec.tree
	local scale = m_min(viewPort.width, viewPort.height) / tree.size * self.zoom
...
function AtlasTreeViewClass:AddNodeName(tooltip, node, build)
...
function AtlasTreeViewClass:AddNodeTooltip(tooltip, node, build)
	...
	if mNode.sd[1] and not mNode.allMasteryOptions then
		tooltip:AddLine(16, "")
		for i, line in ipairs(mNode.stats) do
