    ...
    if node1.g == node2.g and node1.o == node2.o then
        ...
        if arcAngle < m_pi then
            ...
            if arcAngle > (m_pi / 2) then
                ...
                self:BuildArc(arcAngle, node1, node2, secondConnector, true)
            end
            ...
            self:BuildArc(arcAngle , node1, node2, connector)
...
function PassiveTreeClass:BuildArc(arcAngle, node1, node2, connector, mirrorArc)
    ...
    if mirrorArc then
        ...
        --angle = node2.angle - arcAngle - clipAngle -- also works. it's the same angle
    end
    connector.vert = { }
    for _, state in pairs({ "Normal", "Intermediate", "Active" }) do
        ...
        -- calculate the vector of the center of the angle from the origin (group x,y)
        ...
        if mirrorArc then
            vert[3], vert[4] = cX + (size * m_cos(angle) - oX) * p, cY + (size * m_sin(angle) - oY) * p
            vert[7], vert[8] = cX + (size * m_sin(angle) - oX) * p, cY + (size * -m_cos(angle) - oY) * p
        else
