...
local AtlasSpecListClass = newClass("AtlasSpecListControl", "ListControl", function(self, anchor, x, y, width, height, treeTab, specClass)
	self.ListControl(anchor, x, y, width, height, 16, "VERTICAL", true, treeTab.specList)
	self.treeTab = treeTab
	self.controls.copy = new("ButtonControl", {"BOTTOMLEFT",self,"TOP"}, 2, -4, 60, 18, "Copy", function()
		local newSpec = new(specClass, treeTab.build, self.selValue.treeVersion)
	...
	self.controls.new = new("ButtonControl", {"RIGHT",self.controls.rename,"LEFT"}, -4, 0, 60, 18, "New", function()
		local newSpec = new("AtlasSpec", self.treeTab.build, latestTreeVersion)
...
function AtlasSpecListClass:RenameSpec(spec, title, addOnName)
	local controls = { }
...
function AtlasSpecListClass:GetRowValue(column, index, spec)
	if column == 1 then
		...
		return (spec.title or "Default")
	end
end

function AtlasSpecListClass:OnOrderChange()
	self.treeTab.activeSpec = isValueInArray(self.list, self.treeTab.build.atlasSpec)
	self.treeTab.modFlag = true
	self:UpdateItemsTabPassiveTreeDropdown()
end

function AtlasSpecListClass:OnSelClick(index, spec, doubleClick)
	if doubleClick and index ~= self.treeTab.activeSpec then
		self.treeTab:SetActiveSpec(index)
	end
end

function AtlasSpecListClass:OnSelDelete(index, spec)
	if #self.list > 1 then
...
function AtlasSpecListClass:OnSelKeyDown(index, spec, key)
	if key == "F2" then
...
function AtlasSpecListClass:UpdateItemsTabPassiveTreeDropdown()
