...
do
	local lastStat
	for line in io.lines("Gems/statmap.ini") do
...
for _, name in pairs({"act_str","act_dex","act_int","other","sup_str","sup_dex","sup_int"}) do
	local out = io.open("Gems/"..name..".lua", "w")
	for line in io.lines("Gems/"..name..".txt") do
		local spec, args = line:match("#(%a+) ?(.*)")
		if spec then
			if spec == "skill" then
				local grantedId = args
...
os.execute("xcopy Gems\\*.lua ..\\Data\\Gems\\ /Y")
