-- Item data (c) Grinding Gear Games

return {
-- Belt
[[
Ascent From Flesh
Chain Belt
League: Prophecy
Requires Level 44
+(9 to 20) to maximum Energy Shield
+300 to Evasion Rating
+(35 to 45) to Energy Shield
+(10 to 15)% to Elemental Resistances
You have Phasing if Energy Shield Recharge has started Recently
6% chance to Dodge Attacks while Phasing
10% increased Movement Speed while Phasing
]],[[
Auxium
Chain Belt
Requires Level 70
+(9 to 20) to maximum Energy Shield
(10 to 20)% increased Elemental Damage with Weapons
+(45 to 55) to maximum Mana
+(60 to 70) to maximum Energy Shield
0.2% of Physical Attack Damage Leeched as Mana per Power Charge
Chill and Freeze duration on you is based on 65% of Energy Shield
]],[[
Bated Breath
Chain Belt
Requires Level 22
+(9 to 20) to maximum Energy Shield
10% increased Damage
+(15 to 25) to Intelligence
+(20 to 30) to maximum Energy Shield
50% increased Energy Shield Recharge Rate
]],[[
Belt of the Deceiver
Heavy Belt
Requires Level 20
+(25 to 35) to Strength
10% reduced Chance to Block Attacks and Spells
(15 to 25)% increased Physical Damage
You take 30% reduced Extra Damage from Critical Strikes
+(30 to 40) to maximum Life
+(6 to 10)% to all Elemental Resistances
]],[[
Doryani's Invitation
Heavy Belt
Variant: Physical
Variant: Fire
Variant: Cold
Variant: Lightning
Requires Level 68
+(25 to 35) to Strength
{variant:1}(20 to 30)% increased Physical Damage
{variant:2}(20 to 30)% increased Fire Damage
{variant:3}(20 to 30)% increased Cold Damage
{variant:4}(20 to 30)% increased Lightning Damage
{variant:2,3,4}+(300 to 350) to Armour
{variant:1,3,4}+(30 to 35)% to Fire Resistance
{variant:1,2,4}+(30 to 35)% to Cold Resistance
{variant:1,2,3}+(30 to 35)% to Lightning Resistance
{variant:1}0.2% of Physical Damage Leeched as Life
{variant:2}0.2% of Fire Damage Leeched as Life
{variant:3}0.2% of Cold Damage Leeched as Life
{variant:4}0.2% of Lightning Damage Leeched as Life
{variant:1}Your Flasks grant 25% reduced Enemy Stun Threshold during flask effect
{variant:2}Your Flasks grant 10% chance to Ignite during flask effect
{variant:3}Your Flasks grant 10% chance to Freeze during flask effect
{variant:4}Your Flasks grant 10% chance to Shock during flask effect
]],[[
Dyadian Dawn
Heavy Belt
Requires Level 52
+(25 to 35) to Strength
+(70 to 85) to maximum Life
+(20 to 40)% to Fire Resistance
+(20 to 40)% to Cold Resistance
0.6% of Attack Damage Leeched as Life against Chilled enemies
Enemies Ignited by an Attack Burn 20% faster
Deal no Physical Damage
]],[[
Faminebind
Rustic Sash
League: Talisman Standard, Talisman Hardcore
Requires Level 18
(12 to 24)% increased Physical Damage
+(20 to 30)% to Cold Resistance
60% increased Flask effect duration
Deals 50 Chaos Damage per second to nearby Enemies
20% increased Projectile Damage
30% reduced Flask Charges gained while using a Flask
]],[[
Feastbind
Rustic Sash
League: Talisman Standard, Talisman Hardcore
Requires Level 11
(12 to 24)% increased Physical Damage
Adds 5 to 10 Physical Damage to Attacks
+(20 to 40) to maximum Life
0.2% of Physical Attack Damage Leeched as Life
50% increased Flask Charges gained while using a Flask
50% increased Mana Regeneration while using a Flask
]],[[
Headhunter
Leather Belt
League: Nemesis
Requires Level 40
+(25 to 40) to maximum Life
+(40 to 55) to Strength
+(40 to 55) to Dexterity
+(50 to 60) to maximum Life
(20 to 30)% increased Damage against Rare monsters
When you Kill a Rare monster, you gain its mods for 20 seconds
]],[[
Immortal Flesh
Leather Belt
Requires Level 50
+(25 to 40) to maximum Life
+(75 to 100) to maximum Life
(67 to 75) Life Regenerated per second
(8 to 10) Mana Regenerated per second
−5% to all maximum Resistances
−(50 to 40) Physical Damage taken from Attacks
40% increased Armour while not Ignited, Frozen or Shocked
]],[[
The Magnate
Studded Belt
Requires Level 16
(20 to 30)% increased Stun Duration on Enemies
(25 to 40)% increased Physical Damage
+(40 to 50) to Strength
50% increased Flask Charges gained
]],[[
Maligaro's Restraint
Chain Belt
Requires Level 44
+(9 to 20) to maximum Energy Shield
Adds (1-1 to 30-50) Lightning Damage to Attacks
100% increased Shock Duration on You
Shocks you cause are reflected back to you
60% increased Damage while Shocked
15% increased Movement Speed while Shocked
]],[[
Meginord's Girdle
Heavy Belt
Variant: Pre 2.0.0
Variant: Current
Requires Level 8
+(25 to 35) to Strength
{variant:1}Adds 10 to 20 Physical Damage to Attacks
{variant:2}Adds 5 to 15 Physical Damage to Attacks
+25 to Strength
10% increased maximum Life
+(10 to 20)% to Cold Resistance
25% increased Flask Life Recovery rate
]],[[
Perandus Blazon
Cloth Belt
Variant: Pre 1.1.0
Variant: Current
Requires Level 16
(15 to 25)% increased Stun Recovery
+(20 to 30) to all Attributes
{variant:1}(8 to 12)% increased Quantity of Items found
{variant:2}(6 to 8)% increased Quantity of Items found
+20% to Fire Resistance
20% increased Flask effect duration
−2 Physical Damage taken from Attacks
]],[[
Prismweave
Rustic Sash
Requires Level 25
(12 to 24)% increased Physical Damage
10% increased Elemental Damage with Weapons
Adds (3-4 to 7-8) Fire Damage to Attacks
Adds (2-3 to 5-7) Cold Damage to Attacks
Adds (1-1 to 13-17) Lightning Damage to Attacks
+(6 to 8)% to all Elemental Resistances
30% increased Elemental Damage with Weapons while using a Flask
]],[[
The Retch
Rustic Sash
League: Talisman Standard, Talisman Hardcore
Requires Level 44
(12 to 24)% increased Physical Damage
+(60 to 80) to maximum Life
+(25 to 40)% to Cold Resistance
0.4% of Physical Attack Damage Leeched as Life
60% increased Flask Effect Duration
30% reduced Flask Charges gained while using a Flask
200% of Life Leech applies to enemies as Chaos Damage
15% increased Movement Speed while using a Flask
]],[[
Soulthirst
Cloth Belt
Requires Level 45
(15 to 25)% increased Stun Recovery
+(60 to 80) to maximum Life
+15% to all Elemental Resistances
(20 to 30)% increased Mana Recovery from Flasks
(20 to 30)% reduced Flask effect duration
Gain Soul Eater during Flask Effect
Lose Souls gained from Soul Eater on Flask Use
]],[[
Sunblast
Cloth Belt
Requires Level 37
(15 to 25)% increased Stun Recovery
(30 to 40)% increased Trap Damage
20% increased Mana Regeneration Rate
+(20 to 30)% to Fire Resistance
80% reduced Trap Duration
25% increased Light Radius
Traps trigger at the end of their Duration
]],[[
Umbilicus Immortalis
Leather Belt
League: Perandus
Requires Level 30
+(25 to 40) to maximum Life
(8 to 12)% increased maximum Life
2% of Life Regenerated per second
Flasks do not apply to You
Flasks apply to your Zombies and Spectres
]],[[
Wurm's Molt
Leather Belt
Requires Level 8
+(25 to 40) to Maximum Life
+(20 to 30) to Strength
+(20 to 30) to Intelligence
+(10 to 20)% to Cold Resistance
0.4% of Physical Attack Damage Leeched as Life
0.4% of Physical Attack Damage Leeched as Mana
]],
}